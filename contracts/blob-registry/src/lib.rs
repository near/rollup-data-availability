use events::{log_blob_submission, log_maintainer_inclusion, log_owner_transfer};
...
const MINIMUM_DEPOSIT: u8 = 1; // in yoctoNEAR
...
impl Contract {
        ...
        match self.check_authorized(namespace) {
            Some(mut metadata) => {
                ...
                if metadata.maintainers.insert(maintainer.clone()) {
                    log_maintainer_inclusion(maintainer);
                };
            }
            None => {
    ...
    pub fn submit(&self, namespace: Namespace, blob: Vec<u8>) {
        ...
        match self.check_authorized(namespace) {
            Some(_) => {
                // TODO: apart from logging the blob submission, should there
                // something else be done here?
                env::input()
                    .is_none()
                    .then(|| env::panic_str(ERR_INVALID_INPUT));
                log_blob_submission(namespace, blob);
            }
            None => {
                env::panic_str(ERR_UNAUTHORIZED_CALLER);
            }
    ...
    pub fn transfer_ownership(&mut self, new_owner_id: AccountId) {
        ...
        log_owner_transfer(self.own_get_owner(), Some(new_owner_id));
    }
